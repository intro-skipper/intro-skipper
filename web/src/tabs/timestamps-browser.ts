...
export function createTimestampsBrowser(container: HTMLElement): { destroy: () => void } {
  ...
  async function navigateToLibraries(): Promise<void> {
    ...
    try {
      ...
      for (const lib of libraries) {
        const countEl = el("span", { className: "ts-episode-runtime" }, "Loading items\u2026");
      ...
      contentEl.append(
        createStatusMessage(
          "Failed to load libraries: " + (err instanceof Error ? err.message : "Unknown error"),
          "var(--is-error)",
        ),
      );
    } finally {
  ...
  async function navigateToShows(libraryId: string, libraryName: string): Promise<void> {
    ...
    if (!libShows) {
      ...
      try {
        ...
        contentEl.append(
          createStatusMessage(
            "Failed to load shows: " + (err instanceof Error ? err.message : "Unknown error"),
            "var(--is-error)",
          ),
        );
        return;
  ...
  async function navigateToShow(show: ShowItem): Promise<void> {
    ...
    try {
      ...
      contentEl.append(
        createStatusMessage(
          "Failed to load seasons: " + (err instanceof Error ? err.message : "Unknown error"),
          "var(--is-error)",
        ),
      );
    } finally {
  ...
  async function loadSeasonEpisodes(show: ShowItem, season: SeasonItem): Promise<void> {
    ...
    nav$.showDashboardLoading();
    try {
      const { episodes, timestamps } = await tsData.getEpisodesWithTimestamps(show.Id, season.Id);
      ...
      if (!nav$.isCurrentPanel(panelToken)) return;
      epList.setStatus(
        "Failed to load episodes: " + (err instanceof Error ? err.message : "Unknown error"),
        "var(--is-error)",
      );
    } finally {
  ...
  async function loadMovieEpisodes(show: ShowItem): Promise<void> {
    ...
    try {
      ...
      if (!nav$.isCurrentPanel(panelToken)) return;
      epList.setStatus(
        "Failed to load timestamps: " + (err instanceof Error ? err.message : "Unknown error"),
        "var(--is-error)",
      );
    } finally {
      if (nav$.isCurrentPanel(panelToken)) {
