...
public class TestFFmpegService
{
    ...
    public async Task TestNoTrailingOptionsWithBlackIntervalDetection()
    {
        ...
        var blackIntervals = await CreateFFmpegService().DetectBlackIntervalsAsync(episode, 32);

        Assert.NotNull(blackIntervals);
        RunFFmpegAndVerifyNoWarning("-hide_banner -threads 0 -loglevel warning -ss 0 -i ../../../video/credits.mp4 -to 5 -an -dn -sn -vf blackdetect=d=0.1:pix_th=0.0731 -f null -");
    }
