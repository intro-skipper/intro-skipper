...
public class TestLeadInClips
{
    ...
    public async Task SubjectMovingOnAfterTheBackgroundDrops_KeepsTheKeyframeStart()
    {
        ...
        var credits = await CreditsOf(FfmpegTestHelpers.QueueFile("video/moving-story.mp4"));
    ...
    public async Task CutToBlackBetweenKeyframes_StartsAtTheCut()
    {
        ...
        Assert.Equal(17.5, (await CreditsOf(clip))?.Start ?? -1, 3);
        ...
        try
        {
            ...
            Assert.Equal(17.5, (await CreditsOf(remuxed, fingerprintStart: 3.999))?.Start ?? -1, 3);
        }
        finally
        {
            File.Delete(ts);
    ...
    private static async Task<Segment?> CreditsOf(QueuedEpisode episode, double fingerprintStart = 0)
    {
        episode.Duration = 39;
