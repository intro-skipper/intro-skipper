...
public class TestBlackFrames
{
    ...
    [Fact]
    public void TestAdaptiveDensity_FewScenes_ReturnsDefaultThreshold()
    {
        var metrics = new[]
        {
            new CreditSceneMetrics(100, 30),
            new CreditSceneMetrics(100, 40),
        };

        var minimumDensity = CreditDetectionPolicy.ComputeMinimumBlackFrameDensity(metrics);

        Assert.Equal(0.50, minimumDensity);
    }
    ...
    public void TestAdaptiveDensity_LowDensityScenes_RelaxesThreshold()
    {
        var metrics = new[]
        {
            new CreditSceneMetrics(100, 28),
            new CreditSceneMetrics(100, 30),
            new CreditSceneMetrics(100, 32),
        };

        var minimumDensity = CreditDetectionPolicy.ComputeMinimumBlackFrameDensity(metrics);

        Assert.Equal(0.18, minimumDensity, precision: 12);
    }

    [Fact]
    public void TestAdaptiveDensity_HighDensityScenes_KeepsDefaultThreshold()
    {
        var metrics = new[]
        {
            new CreditSceneMetrics(100, 85),
            new CreditSceneMetrics(100, 90),
            new CreditSceneMetrics(100, 95),
        };
    ...
    [Fact]
    public void TestAdaptiveDensity_EmptyMetricsExcluded_DoNotSkewMedian()
    {
        var metrics = new[]
        {
            new CreditSceneMetrics(0, 0),
            new CreditSceneMetrics(0, 0),
            new CreditSceneMetrics(100, 28),
            new CreditSceneMetrics(100, 30),
            new CreditSceneMetrics(100, 32),
    ...
    public void TestAdaptiveDensity_EvenCount_TrueMedianUsed()
    {
        var metrics = new[]
        {
            new CreditSceneMetrics(100, 20),
            new CreditSceneMetrics(100, 40),
            new CreditSceneMetrics(100, 60),
            new CreditSceneMetrics(100, 80),
        };

        var minimumDensity = CreditDetectionPolicy.ComputeMinimumBlackFrameDensity(metrics);

        Assert.Equal(0.30, minimumDensity, precision: 12);
    }

    [Fact]
    public void TestDetectCreditScenes_LowDensityAdaptiveThresholdAllowsRepeatedScenes()
    {
        var frames = new List<BlackFrame>();
        ...
        BlackInterval[]? intervals = null) : IFFmpegService
    {
        ...
        public Task<BlackInterval[]> DetectBlackIntervalsAsync(QueuedEpisode episode, int threshold, CancellationToken cancellationToken = default)
            => DetectBlackIntervalsAsync(episode, new TimeRange(episode.CreditsFingerprintStart, episode.CreditsFingerprintEnd), threshold, cancellationToken);

        public Task<BlackInterval[]> DetectBlackIntervalsAsync(QueuedEpisode episode, TimeRange range, int threshold, CancellationToken cancellationToken = default)
        {
            IntervalScanCalls++;
