...
/// <summary>
/// Pins the strict-overlap rule that drives every automatic-write guard in
...
/// </summary>
public sealed class TestSegmentWritePolicy
{
    [Theory]
    ...
    {
        Assert.Equal(expected, SegmentWritePolicy.Overlaps(aStart, aEnd, bStart, bEnd));

        // The guard compares incoming/stored ranges in one fixed argument order; the
        // predicate must not care which side is which.
        Assert.Equal(expected, SegmentWritePolicy.Overlaps(bStart, bEnd, aStart, aEnd));
    }
}
