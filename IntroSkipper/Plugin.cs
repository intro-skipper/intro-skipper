...
public partial class Plugin : BasePlugin<PluginConfiguration>, IHasWebPages
{
        ...
        : base(applicationPaths, xmlSerializer)
    {
        ...
        _keyframeRepository = ResolveKeyframeRepository(applicationHost, serviceProvider, logger);
    ...
    private static IKeyframeRepository? ResolveKeyframeRepository(IApplicationHost applicationHost, IServiceProvider serviceProvider, ILogger logger)
    {
        try
        {
            var implType = applicationHost.GetExportTypes<IKeyframeRepository>().FirstOrDefault();
            if (implType is null)
            {
                logger.LogDebug("IKeyframeRepository implementation not found; keyframe caching disabled.");
                return null;
            }

            var repo = serviceProvider.GetService(typeof(IKeyframeRepository)) as IKeyframeRepository;
            if (repo is null)
            {
        ...
        {
            logger.LogDebug(ex, "Jellyfin.MediaEncoding.Keyframes not available; keyframe caching disabled.");
            return null;
        }
    }
    ...
    {
        return _keyframeRepository?.GetKeyframeData(id) ?? [];
    }
